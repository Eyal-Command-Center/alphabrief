  ...
  async function handlePromo(e: React.FormEvent) {
    ...
    if (!res.ok) {
      ...
      try {
        const cached = localStorage.getItem('ab_email_prefs')
        const prev = cached ? JSON.parse(cached) : {}
        localStorage.setItem('ab_email_prefs', JSON.stringify({ ...prev, is_pro: true }))
      } catch {}
    }
              ...
              <div className="relative flex flex-col h-full">
                ...
                {!showPromo ? (
                  <button
                    ...
                    className="w-full text-center text-slate-600 hover:text-slate-400 text-xs mt-2 transition-colors"
                  >
                    Have a promo code?
                  </button>
                ) : promoSuccess ? (
                  <p className="text-center text-emerald-400 text-xs mt-2 font-semibold">✓ Pro activated!</p>
                ) : (
                  <form onSubmit={handlePromo} className="mt-3 flex gap-2">
                    <input
                      type="text"
                      value={promoCode}
                      onChange={e => { setPromoCode(e.target.value); setPromoError('') }}
                      placeholder="Enter code"
                      className="flex-1 bg-slate-800 border border-white/10 rounded-lg px-3 py-2 text-white text-xs placeholder-slate-600 focus:outline-none focus:border-emerald-500/50"
                    />
                    <button
                      type="submit"
                      disabled={promoLoading || !promoCode.trim()}
                      className="bg-slate-700 hover:bg-slate-600 disabled:opacity-50 text-white text-xs font-semibold px-3 py-2 rounded-lg transition-all"
                    >
                      {promoLoading ? '…' : 'Apply'}
                    </button>
                  </form>
                ...
                {!showPromo && <p className="text-slate-600 text-xs text-center">Cancel anytime. Billed monthly.</p>}
              </div>
            </div>
          ) : (
