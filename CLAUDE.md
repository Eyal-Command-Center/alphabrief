...
AlphaBrief has a living services reference: `AlphaBrief-Services-Reference.docx` at the repo root.
The data lives in `scripts/build-services-doc.js`. Claude maintains both directly — no GitHub Action, no auto-build.

**BEFORE COMMITTING any of the following changes, Claude MUST update the doc:**

| Change | What to update in build-services-doc.js |
|---|---|
| New/removed external service or API key | `services` array + `envVars` array |
| New/removed API route | `routes` array |
| New/removed Supabase table | `dbTables` array |
| New/removed page | `pages` array |
| Architecture change (caching, auth, data flow) | `arch` array |
| Feature shipped or deferred | `built` or `pending` arrays |
| Model name, cron schedule, billing plan change | relevant array entry |
